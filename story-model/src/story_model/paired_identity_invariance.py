...
FULL_SUPERVISION_MODE = "full_supervision"
CLEAN_ANCHOR_SUPERVISION_MODE = "clean_anchor"
...
def mask_swapped_supervision(
    batch: tuple[torch.Tensor, ...],
    original_row_count: int,
) -> tuple[torch.Tensor, ...]:
    """Remove every supervised target from the swapped (paired) rows.

    Phase 36d's clean-anchor objective: the swapped copy receives no direct
    tag/type/boundary loss of its own, only the original ("anchor") rows do.
    The swapped copy still participates in the forward pass and in the JSD
    consistency term via its logits, just not in ``output.loss``.

    Because the model's cross-entropy losses use ``ignore_index`` (see
    ``ExplicitOffsetCandidateProposer.forward``), a fully ``IGNORE_TAG``-masked
    row contributes to neither the loss sum nor its normalizing count. This
    makes ``output.loss`` on the augmented batch mathematically identical to
    computing it on the original batch alone, which is what the Phase 36d
    lambda=0 regression test verifies.
    """
    tokens, sequence_lengths, prompt_token_counts, tag_targets = batch
    if original_row_count == tag_targets.shape[0]:
        return batch
    masked_targets = tag_targets.clone()
    masked_targets[original_row_count:] = IGNORE_TAG
    return tokens, sequence_lengths, prompt_token_counts, masked_targets
