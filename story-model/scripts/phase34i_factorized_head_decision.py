"""Dependency-free decision logic for Phase 34i factorized heads."""
...
def factorized_head_decision(baseline: dict, candidate: dict) -> dict:
    """Decide whether shared boundaries fix the held-out route starts."""
    ...
    for geometry in (
        "checkpoint_token_width_geometry_version",
        "checkpoint_token_end_geometry_version",
    ):
        if baseline.get(geometry) is not None:
            provenance_failures.append(
                f"baseline unexpectedly declares {geometry}"
            )
        if candidate.get(geometry) is not None:
            provenance_failures.append(
                f"factorized candidate unexpectedly declares {geometry}"
            )
    if baseline.get("checkpoint_factorized_boundary_type_version") is not None:
        provenance_failures.append(
            "baseline unexpectedly contains factorized boundary/type heads"
        )
    if candidate.get("checkpoint_factorized_boundary_type_version") != (
        EXPECTED_FACTORIZED_VERSION
    ):
        provenance_failures.append(
            "candidate does not declare factorized boundary/type version 1"
        )
    ...
    width_two = dimensions.get("token_width", {}).get("2")
    ...
    token_buckets = dimensions.get("token_id", {})
    token_diagnostics = {
        str(token_id): token_buckets.get(str(token_id))
        for token_id in DIAGNOSTIC_TOKEN_IDS
    }
    ...
    if provenance_failures:
        branch = "invalid_factorized_head_comparison"
        next_action = (
            "Stop and correct the checkpoints or audit inputs; do not "
            "interpret this comparison."
        )
    elif not safety_passed:
        branch = "factorized_head_rejected"
        next_action = (
            "Reject factorization because a retained boundary or type cell "
            "regressed. Do not run the full gate or extend training."
        )
    elif target_passed and checkpoint_eligible:
        branch = "factorized_head_diagnostic_pass"
        next_action = (
            "Run the unchanged full Phase 34 proposer and Phase 33c oracle "
            "gates. Promote only if both pass."
        )
    elif target_passed:
        branch = "factorized_head_span_fixed_but_ineligible"
        next_action = (
            "Factorization fixed the registered boundary cells, but the "
            "training panel is ineligible. Stop without a full gate, "
            "checkpoint promotion, or longer training."
        )
    else:
        branch = "factorized_head_insufficient"
        next_action = (
            "The shared boundary head did not fix held-out route starts. "
            "Stop without extending training; design a training-only "
            "compositional boundary counterbalance and refresh the held-out "
            "values before testing it."
        )
    return {
        "branch": branch,
        ...
        "diagnostic_token_identities": token_diagnostics,
        "diagnostic_token_identities_are_gate_inputs": False,
